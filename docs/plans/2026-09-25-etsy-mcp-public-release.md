...
**Plan:** define a small platform matrix for Windows Credential Manager, macOS Keychain, and Linux Secret Service. Use synthetic credentials only. Verify probe, save, read, token refresh, clear, locked/denied/unavailable store, and write failure. Assert that errors are useful, no plaintext fallback exists, and no credential values appear in logs or process arguments. Run the integration lane on native hosted runners (Linux with Secret Service available); keep unit tests separate from native integration evidence.
