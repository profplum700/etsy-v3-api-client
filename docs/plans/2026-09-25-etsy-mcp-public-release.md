...
8. Restore sequential recovery behavior for callers using the public `waitForRateLimit()` / `updateFromHeaders(headers)` convenience pair. Preserve the stricter reservation-aware behavior used by SDK internals so delayed responses cannot reopen exhausted quota. Add a regression that exhausts QPD, advances to an uncorrelated convenience probe response with positive remaining quota, and verifies the limiter recovers; keep the existing stale correlated response regression passing.
