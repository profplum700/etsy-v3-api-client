...
**Review-fix acceptance:** all five inline review/CodeQL threads receive a direct response after their changes are present; the Codecov conversation receives a direct response; every required thread is re-fetched and verified. CI, CodeQL, and all OS keyring/lock matrix jobs pass on the final PR head. The tag workflow has deterministic acceptance checks but is not triggered against a real release tag during review-fix work; no npm/Registry publication occurs here.
